...
//#include "AliCFContainer.h" //error on grid
...
void AliAnalysisMBVeto::CreateOutputObjects()
{
 ...
 //   fTreeTrack->Branch("fastOR",&fastORHW,"fastOR/I"); //
  ...
  //  fTreeTrack->Branch("t0PU",&t0PileUp,"t0PU/I"); // pile-up from T0
    ...
    TH2F *hTotalTrkVsClsSPID = new TH2F("hTotalTrkVsClsSPID","; Spd : total",140,0,140,1000,0,1000);
    ...
    TH2F *hTotalAD = new TH2F("hTotalAD","; AD : total",2000,-100,100,2000,-100,100);
    ...
    for(int i=0; i<3; i++){
       ...
       hDenomV0[i][j][k]->GetYaxis()->SetTitle("V0A+V0C");
                ...
                hADNumTrkVsClsSPD[i][j][k] = new TH2F(Form("hADNumTrkVsClsSPD%d_AD%d_Flag%d",i,j,k),"; Spd : !BGid & GoodEvent",140,0,140,1000,0,1000);
                hADNumTrkVsClsSPD[i][j][k]->GetXaxis()->SetTitle("Tracklet");
                hADNumTrkVsClsSPD[i][j][k]->GetYaxis()->SetTitle("Cluster (fspdC1)");
                
                hNumAD[i][j][k] = new TH2F(Form("hNumAD%d_AD%d_Flag%d",i,j,k),"; AD : !BGid & GoodEvent",3000,-300,300,3000,-300,300);
                hNumAD[i][j][k]->GetXaxis()->SetTitle("ADA-ADC");
                hNumAD[i][j][k]->GetYaxis()->SetTitle("ADA+ADC");
                hDenomAD[i][j][k]= new TH2F(Form("hDenomAD%d_AD%d_Flag%d",i,j,k),"; AD : !BGid",3000,-300,300,3000,-300,300);
 ...
 //   SPDHw2 = triggerAnalysis->SPDFiredChips(fESD,1,kFALSE,2);  //SPD Fired Chips in layer 2 (from hardware bit)
  //  t0PileUp = triggerAnalysis->EvaluateTrigger(fESD, (AliTriggerAnalysis::Trigger) (AliTriggerAnalysis::kOfflineFlag | AliTriggerAnalysis::kT0Pileup)); //T0 pile-up
   ...
   // Bool_t goodEvent;
    ...
    if(ftrigger[0]) {  // trigger class
        
        ((TH1F*)fList->FindObject("hTotalTrkVsClsSPID"))->Fill(fSpdT, fSpdC1);
                    ...
                    for(int k=0; k<3; k++){
                        ...
                        if(!bgID) {
                            ...
                            if(SelGoodEventAD[i][j][k]){
                                ...
                                ((TH1F*)fList->FindObject(Form("hADNumTrkVsClsSPD%d_AD%d_Flag%d",i,j,k)))->Fill(fSpdT, fSpdC1);
